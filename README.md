...
**현재 상태:** **M8 콘텐츠** — 보스 자동 재도전, 업적, 동료 승급까지. 다음은 장비, 2차 환생. 효과음은 맨 마지막
...
```
autoload/   Balance(수치·공식, balance/ 부분 스크립트를 상속으로 연결) · Num(한국식 숫자 표기) · Game(골드·스테이지·몬스터·보스) · Party(용사·동료 레벨과 구매) · Skills(스킬) · Training(단련) · Promotions(동료 승급) · Prestige(회귀·기억의 상점) · Achievements(누적 통계·업적·영구 보너스) · Save(저장·불러오기·오프라인 보상)
scenes/     main.tscn 세로 화면 전체 · top_bar · battle/ 전투 화면 · skill_bar · nav_bar 탭 버튼 · toast 달성 알림 · tabs/ 탭 패널과 구매 배수
tests/      헤드리스 테스트 러너와 스위트 (공식, 진행, 보스, 저장, 오프라인, 스킬, 회귀, 단련, 승급, 업적)
...
| M8 | 콘텐츠: 보스 자동 재도전 (완료), 업적 32종과 영구 보너스 (완료), 동료 승급 5단계 (완료), 장비, 2차 환생 | 진행 중 |
