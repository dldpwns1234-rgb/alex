...
**현재 상태:** **M3 저장과 오프라인** — 자동 저장, 내보내기·가져오기, 오프라인 보상
...
```
autoload/   Balance(수치·공식) · Num(한국식 숫자 표기) · Game(골드·스테이지·몬스터·보스) · Party(용사·동료 레벨과 구매) · Save(저장·불러오기·오프라인 보상)
scenes/     main.tscn 세로 화면 전체 · battle/ 전투 화면 · tabs/ 탭 패널
tests/      헤드리스 테스트 러너와 스위트 (공식, 진행, 보스, 저장, 오프라인)
