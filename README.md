...
**현재 상태:** **M4 스킬** — 폭풍 베기, 전투의 함성, 황금 손길 (실제 시간 기준 쿨타임)
...
```
autoload/   Balance(수치·공식, balance/ 부분 스크립트를 상속으로 연결) · Num(한국식 숫자 표기) · Game(골드·스테이지·몬스터·보스) · Party(용사·동료 레벨과 구매) · Skills(스킬) · Save(저장·불러오기·오프라인 보상)
scenes/     main.tscn 세로 화면 전체 · top_bar · battle/ 전투 화면 · skill_bar · tabs/ 탭 패널과 구매 배수
tests/      헤드리스 테스트 러너와 스위트 (공식, 진행, 보스, 저장, 오프라인, 스킬)
