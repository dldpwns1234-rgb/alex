...
res://
  autoload/
    ...
    balance/     leveling(용사·레벨업) → companions(동료) → skills(스킬) → memory(회귀·상점) → training(단련) → achievements(업적) → equipment(장비) → balance.gd(몬스터·보스·오프라인). 바깥에서는 Balance.만 쓴다
  ...
  scenes/
    ...
    tabs/        hero(레벨업과 장비 3칸), party, training, prestige, achievements, settings. 스크롤 목록은 tap_scroll(버튼 위에서도 끌어 스크롤, 탭 판정)을 쓴다
