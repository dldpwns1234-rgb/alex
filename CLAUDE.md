...
res://
  autoload/
    ...
    balance/     leveling(용사·레벨업) → companions(동료) → skills(스킬) → memory(회귀·상점) → balance.gd(몬스터·보스·오프라인). 바깥에서는 Balance.만 쓴다
    game.gd      (Game) 골드, 스테이지, 몬스터, 보스. 상태 변경은 Game·Party·Skills·Prestige에서만
