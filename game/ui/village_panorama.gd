...
##
## **세로 화면이므로 마을은 위아래로 흐른다.**
## 좌우 엇갈림으로 굽이진 마을길처럼 보이게 하되 가로 스크롤은 두지 않는다.
## 세로 화면에서 가로로도 밀어야 하면 슬롯 절반이 늘 화면 밖에 있고,
## 무엇이 지어졌는지 한눈에 들어오지 않는다.
##
...
## 세로 화면 폭 720에서 두 열이 여유롭게 들어가는 크기.
const SLOT_SIZE := Vector2(300, 140)
...
func build(layout: Dictionary, slot_count: int) -> void:
	...
	var panorama_size := Vector2(
		...
		float(size_data.get("height", 1240)),
	)
	...
	for slot_index in slot_count:
		var position := Vector2(40.0, 160.0 * slot_index)
...
func _add_path(panorama_size: Vector2) -> void:
	...
	path.position = Vector2(panorama_size.x * 0.5 - 26.0, 0)
	path.size = Vector2(52, panorama_size.y)
...
func _refresh_slot(button: Button, village: SimVillage, slot_index: int) -> void:
	if not village.is_slot_unlocked(slot_index):
		...
		button.text = "잠김 · %d년차" % village.slot_unlock_years[slot_index]
